...
│   │   │   │   └── wallet/      # ✅ Кошелёк: персональные валюты (Currency), неизменяемый Ledger (вычисляемый баланс + материализованный кэш с блокировкой), эскроу наград задач (заморозка→выплата→возврат), сжигание
...
- **WalletModule** ✅: персональные валюты + неизменяемый Ledger + эскроу наград задач. `Currency` (одна активная на эмитента — partial unique; эмитент полиморфен user|workspace, задел под B2B), `LedgerEntry` (append-only, истина), `WalletBalance` (материализованный кэш, блокировка строки → нет двойной траты, пересчитывается), `EscrowHold` (состояние на участника). `LedgerService` (mint с лимитом 10М «на руках»/burn/transfer/reverse/freeze/unfreeze), `CurrencyService` (жизненный цикл валюты + кошелёк/история/держатели + сжигание чужой валюты), `EscrowService` (hold/capture/return/release). Интеграция в `TasksService` синхронно в транзакции (создание→заморозка с блоком без средств, приёмка→выплата + `wallet.coins.received`, возврат после приёмки→реверс в минус + переморозка, отмена/удаление→рефанд). UI: раздел «Кошелёк» в `/profile` + бейдж «держит N 🪙» на карточке в Окружении. B2C готово; компанийная валюта/история-клемнбэк/матрица видимости — позже. Проверено e2e (`apps/api/scripts/verify-{wallet,escrow,burn}.cjs`). Дизайн — Serena memory `project_wallet_module`
...
### Wallet — кошелёк, Ledger, эскроу (`/api/wallet/`) ✅
> Каждый пользователь выпускает ОДНУ личную валюту (имя+эмодзи, без тикера) для награды за задачи. Баланс — производное от **неизменяемого журнала** (`LedgerEntry`, append-only, никогда не UPDATE) + материализованный кэш `WalletBalance` (блокировка строки на списании → нет двойной траты; пересчитывается из журнала). Эмитент **полиморфен** (`issuerType` user|workspace) — компанийная валюта позже на той же схеме. Лимит эмиссии: «на руках» (доступно+заморожено) ≤ 10 млн. Только целые (BigInt; DTO отдаёт Number, < 2^53).
...
**Эскроу с Задачником** (синхронно, в одной транзакции `TasksService`↔`EscrowService` — не через EventBus): награда — только своя валюта. Создание задачи с наградой → **заморозка** по каждому исполнителю (нет монет/валюты → задачу с наградой не создать, откат). Приёмка → **выплата** исполнителю (+ уведомление `wallet.coins.received`). Возврат после приёмки → **реверс** (баланс исполнителя может уйти в минус — «честный реверс») + переморозка (награда остаётся). Отмена/удаление → рефанд создателю. Идемпотентность через `idempotencyKey` + статусы `EscrowHold`. Кошелёк НЕ в chokepoint (личные коины не принадлежат организации). Полиморфный эмитент + `Currency.visibilityPolicy` (JSON) — задел под B2B. Дизайн — Serena memory `project_wallet_module`.
