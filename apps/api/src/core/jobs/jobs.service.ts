...
export class JobsService {
  ...
  async enqueue(tx: Tx | null, input: EnqueueInput): Promise<void> {
    ...
    if (input.uniqueKey) {
      // ВАЖНО: время всегда передаём параметром-Date, НИКОГДА не через SQL now(). Колонки
      // timestamp(3) БЕЗ таймзоны, Prisma пишет в них UTC; now() — timestamptz, и при
      // session TimeZone ≠ UTC Postgres записал бы локальное «стенное» время. Смешение
      // двух часов в одной колонке ломает и видимость (run_at), и reaper (lease_until).
      const stamp = new Date();
      await client.$executeRaw`
        INSERT INTO jobs (type, queue, payload, status, priority, run_at, max_attempts, unique_key, created_at, updated_at)
        VALUES (${input.type}, ${queue}, ${JSON.stringify(input.payload ?? {})}::jsonb, 'available', ${priority}, ${runAt}, ${maxAttempts}, ${input.uniqueKey}, ${stamp}, ${stamp})
  ...
  async claim(queue: string, types: string[], limit: number): Promise<ClaimedJob[]> {
    ...
    // Время — только параметрами-Date (см. комментарий в enqueue про now() и таймзону).
    const now = new Date();
    ...
    >`
      WITH picked AS (
        SELECT id FROM jobs
        WHERE status = 'available' AND queue = ${queue} AND run_at <= ${now}
  ...
  async reapExpired(): Promise<void> {
    // Время — параметрами-Date (см. enqueue): now() в naive-колонках даёт локальное время.
    const now = new Date();
    const dead = await this.db.$queryRaw<
      Array<{ id: bigint; type: string; attempts: number; payload: unknown; lastError: string | null }>
    >`
      UPDATE jobs
      SET status = 'discarded', lease_until = NULL, finished_at = ${now}, updated_at = ${now},
          last_error = COALESCE(last_error, 'аренда истекла (краш инстанса?)')
      WHERE status = 'executing' AND lease_until < ${now} AND attempts >= max_attempts
    ...
    const requeued = await this.db.$executeRaw`
      UPDATE jobs
      SET status = 'available', lease_until = NULL, updated_at = ${now},
          run_at = ${now}::timestamp + make_interval(secs => LEAST(
            ${JOB_LIMITS.backoffCapMs / 1000}::float8,
            ${JOB_LIMITS.backoffBaseMs / 1000}::float8 * POWER(2, GREATEST(attempts, 1) - 1)
          ))
  ...
  async stats(): Promise<JobStatsDto> {
    ...
    const oldest = await this.db.job.findFirst({
      where: { status: 'available', runAt: { lte: new Date() } },
      orderBy: { runAt: 'asc' },
      select: { runAt: true },
    });
