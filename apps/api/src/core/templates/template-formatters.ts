...
/**
 * Месяцы БЛАНКА, а не интерфейса. Это платформенный КОНТЕНТ: приказ по ТК РК
 * печатается словами того языка, на котором составлен сам документ, и его
 * перевод — отдельный трек с юридической вычиткой, а не строка каталога.
 * Поэтому список живёт здесь, рядом с DSL шаблонов, а не в общем реестре:
 * иначе он выглядел бы как «подпись интерфейса» и уехал бы в переводы.
 */
...
/**
 * Язык ДОКУМЕНТА (крюк мультиязычности). Сегодня форматтеры знают только
 * русский — бланки РК на казахском требуют своих склонений и «прописью» по
 * правилам казахского числительного; это отдельный трек. Параметр введён
 * СЕЙЧАС, чтобы вызывающие (Документы, КЭДО) уже передавали язык шаблона и
 * второй раз менять их сигнатуры не пришлось.
 */
export type TemplateLanguage = 'ru' | 'kk';
 ...
 *
 * `language` — язык САМОГО БЛАНКА (не интерфейса зрителя): один и тот же
 * приказ печатается на языке, на котором составлен, кто бы его ни открыл.
 */
export function applyFormatterChain(
  value: unknown,
  chain: { key: string; arg?: string }[],
  tagHint: string,
  language: TemplateLanguage = 'ru',
): string {
  // Казахские бланки — отдельный трек (склонения месяцев, «прописью» по
  // правилам казахского числительного). Пока язык только принимается.
  void language;
