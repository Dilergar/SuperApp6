...
export class VerifyService {
  ...
  /**
   * Простая электронная подпись под КОНКРЕТНЫМ документом (core/sign).
   * ВЫЗЫВАЕТСЯ ТОЛЬКО движком подписи, который уже проверил: акт принадлежит
   * этому человеку, заявка жива, соглашение сторон принято. Публичный
   * /verify/start эту цель отвергает — иначе он стал бы SMS-пушкой.
   *
   * Пароль здесь НЕ спрашивается осознанно, и это не послабление: ст. 46 п. 4 ЦК
   * говорит «после цифровой аутентификации» — вход в систему и есть первый
   * фактор, а код под документ — второй. Требовать пароль на каждую подпись
   * значит сделать подписание десяти приказов подряд невыносимым, ничего не
   * добавив: тот, у кого есть сессия, пароль тоже знает.
   *
   * Номер приходит НЕ от клиента: у сотрудника он берётся из его аккаунта, у
   * гостя — из подтверждённой записи ShareLinkGuest. Поэтому гео-щит здесь не
   * нужен (пампинг этой целью не сделать).
   */
  async startForSign(phone: string, userId: string | null, ip?: string): Promise<VerifyStartResponse> {
