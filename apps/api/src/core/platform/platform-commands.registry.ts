...
export interface CommandOutcome {
  ...
  /**
   * Эффект команды ПОСЛЕ коммита её транзакции (сброс кэшей движка-владельца): та же гонка,
   * что у `invalidateStaff`, — сброс внутри транзакции возвращает в кэш старое состояние.
   * Предпросмотр (откатываемая транзакция) хук не зовёт. Ошибка хука команду не роняет.
   */
