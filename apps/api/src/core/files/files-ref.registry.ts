...
export interface FileRefOptions {
  ...
  anchorOnly?: boolean;
  /**
   * У ЭТОГО места СВОИ правила видимости, и они СИЛЬНЕЕ общего «файл организации
   * виден всей команде». Появилась такая связь — решает место, а не владение файлом.
   *
   * Иначе закрытая настройка сущности не значит ничего: у документа организации вид
   * может быть «только управляющим», а файл при этом принадлежит организации — и
   * любой Стажёр, узнавший fileId, скачивал бы его напрямую. Первым это правило
   * получил Диск (закрытая папка), теперь оно объявляется потребителем, а не зашито
   * в движок именами типов.
   */
  scopedPlace?: boolean;
}
