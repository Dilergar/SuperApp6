...
export interface JwtPayload {
  ...
  sid?: string;
  /**
   * Семейство refresh-цепочки (`sessions.family_id`) — «сессия» для человека (core/audit):
   * строка ротируется на каждом refresh, семейство — нет. Журнал пишет его в
   * `actor_family_id`; «Это не я» и cooling-гард читают его. Токены до поля — без него.
   */
  fam?: string;
  /**
