...
async function call(method, p, token, body) {
  const res = await fetch(BASE + p, {
    method,
    headers: { 'Content-Type': 'application/json', 'Idempotency-Key': require('crypto').randomUUID(), 'X-Locale': process.env.SA6_SUITE_LOCALE || 'ru', 'X-Device-Id': require('./_lib.cjs').SUITE_DEVICE_ID, ...(token ? { Authorization: 'Bearer ' + token } : {}) },
