...
// Nest-граф. Появится новая таблица с workspace_id — править ОБА места. Данные движков и
// сервисов (Диск, заметки, документы, файлы, ссылки наружу, заявки) скрипт НЕ трогает —
// их после --apply добирает штатный каскад: gc-orphan-workspace-data.cjs --apply.
...
async function main() {
  ...
  try {
    ...
    console.log('Данные движков этих организаций (Диск, заметки, файлы, ссылки): node scripts/gc-orphan-workspace-data.cjs --apply');
  } finally {
