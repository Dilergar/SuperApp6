...
const nextConfig: NextConfig = {
  ...
  transpilePackages: ['@superapp/shared', '@superapp/api-client', '@superapp/i18n', '@superapp/analytics'],
  experimental: {
  ...
  async headers() {
    return [
      {
        // Пак значков: 1595 картинок эмодзи, сабсеты шрифта и два каталога.
        // Next по умолчанию отдаёт /public без кэша (ревалидация на каждый
        // заход), а здесь содержимое привязано к пинам сборки и меняется только
        // вместе с ними — держим год и immutable, иначе сетка выбора каждый раз
        // упирается в сотню условных запросов.
        source: '/glyphs/:path*',
