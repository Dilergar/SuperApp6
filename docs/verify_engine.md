...
**`SmsOutboundService`** — служебные SMS потребителям (ссылка на подпись контрагенту): гео-щит, кулдаун 60с, суточный потолок организации. Канал `sms` движка уведомлений ([notifications_engine.md](notifications_engine.md)) берёт тот же драйвер `VerifySmsService` напрямую (critical + opt-in человека, свои суточные потолки в Redis по этому образцу).
