...
- **Системная папка размещения**: `resolvePlacement` может вернуть `folder` (ключ `DRIVE_SYSTEM_FOLDERS`, по умолчанию «Файлы из переписки»). Папка с `restricted: true` и `grantWorkspaceRole` создаётся закрытой (`systemEnsureRestricted`) и получает грант `viewer` роли организации — так выгрузки журнала безопасности (`security_exports`, «Безопасность», грант — admin) видит только руководство. Ссылки ядра, у которых нет модуля-владельца на Диске, регистрирует `DriveCoreRoutesProvider` (`audit_export` → Диск организации) — [audit_engine.md](audit_engine.md).
...
- **Окончательное удаление организации** стирает её пространства Диска хуком `drive.workspace` (`drive.lifecycle.provider.ts` → `DriveTreeService.purgeOwnerSpaces` с бюджетом): системный `purge(null, узел)` — гранты, привязки, ссылки наружу, файлы (кроме защищённых личным архивом), индекс, — затем строки пространств (узлы и фото-корзины снимает FK). Корзина 30 дней — шаг `drive.trash` раннера сроков core/lifecycle; «недавно открывал» — общая пачка политики `DriveRecent` (90 дней).
- **`purge` поддерева — пачками «сначала листья»** (`DRIVE_LIMITS.purgeBatch`, самые глубокие узлы первыми, строки — явным списком): папка на десятки тысяч файлов не читается одним запросом, а каскад по parent_id не снесёт узел, чьи файлы и гранты ещё не сняты.
...
- **Автоимя узла** (`auto_name_key` + `auto_name_params`): имя собрала платформа — «ДОГ-2026-001 Договор (подписано).pdf» от документооборота. В `name` остаётся снимок языка-источника (поиск, сортировка, уникальность в папке), а `displayName` собирает имя при чтении в языке зрителя. Человек переименовал — ключ гасится, дальше это ЕГО имя и мы его не трогаем ([i18n.md](i18n.md)).
...
- **`share()` валидирует ЛЮБОЙ принципал** (`assertPrincipalAllowed`): личный диск = человек из окружения и СВОЯ Группа; диск организации = её сотрудник и её оси. + `PersonalGraphRegistry.register('drive')`. Грант, хроника и (на диске организации) событие журнала `sharing.access.granted|revoked` — одной транзакцией; тот же уровень и снятие несуществующего гранта событий не пишут. Гостевой ZIP папки — `data.export{drive_guest_zip}` до первого байта ([audit_engine.md](audit_engine.md)). Подписи получателей (`principalName`) — `AudiencesService.labelMany` ([audiences_engine.md](audiences_engine.md)); относительные виды (`grantable:false`) в шеринг не попадают.
