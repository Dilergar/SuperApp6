...
- **Окончательное удаление организации** стирает её пространства Диска хуком `WorkspacePurgeRegistry` (`drive-workspace-purge.provider.ts` → `DriveTreeService.purgeOwnerSpaces`): системный `purge(null, корни)` — гранты, привязки, ссылки наружу, файлы (кроме защищённых личным архивом), индекс, — затем строки пространств (узлы и фото-корзины снимает FK).
