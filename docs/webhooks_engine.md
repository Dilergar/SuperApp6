...
## Хуки подписки и события безопасности

`WebhooksRegistry.registerSubscriptionHook({ prefix, onChange(tx, {actorId, workspaceId, before, after}) })` — модуль-владелец событий с префиксом `prefix` видит КАЖДОЕ изменение подписки на них (создание, правка событий, удаление) в транзакции этого изменения: может отказать (`402` тарифа) или записать факт. Так тариф проверяется на всех путях, а не на одной ручке. Первый потребитель — журнал безопасности: события `security.<категория>.recorded` (`SECURITY_WEBHOOK_EVENTS`, сервис `security`; категории `org | keys | data | consents | detect`) под тарифом `audit.stream`, смена подписки — факт `org.audit.stream_changed`; доставка — `emit(tx)` из наблюдателя записи В ТРАНЗАКЦИИ факта, payload OCSF без IP, UA и имён — [audit_engine.md](audit_engine.md). Веб: группа «Безопасность» в форме эндпоинта заблокирована замком тарифа.
...
**Видимость полей.** Событие наружу — внешнее назначение: поле записи в payload идёт только через `visibility.forExternal()` (без раскрытия; бот/ключ — класс не выше `internal`, `contact` — с флагом ключа «Доступ к контактным данным»). Сегодня события вебхуков полей служебных типов видимости не несут; имя поля класса `personal` и выше в схеме события ловит `check:visibility` — [visibility_engine.md](visibility_engine.md).
