...
| [data_architecture.md](data_architecture.md) | Архитектура данных: карта хранилищ (PostgreSQL 18 в двух городах, PgBouncer, Redis «состояние» и «кэш», S3), идентификаторы uuid v7, владелец данных и готовность к ячейкам, что партиционировано, подключения (`DATABASE_URL` / `DIRECT_URL` / обслуживающее), потолки ролей, две роли Redis и их правила, пробы и сторожевые метрики, триггеры масштабирования, ловушки |
| [lifecycle_engine.md](lifecycle_engine.md) | Жизненный цикл данных (`core/lifecycle`, 28-й): единый реестр политик ВСЕХ хранилищ (модели, сырые таблицы, байты файлов, семейства Redis, архивы и бэкапы) — класс, владелец, основание, срок (пол закона / умолчание / потолок), стирание субъекта, удаление организации, граф удаления, принуждение; страж `check:lifecycle` и ратчет готовности к ячейкам |
| [lifecycle_erasure.md](lifecycle_erasure.md) | Стирание человека и организации (`core/lifecycle`): оркестратор с квитанцией и подписанным сертификатом (Ed25519), обязательные вебхуки redact, заморозки (legal hold) на всех путях удаления + hold store, «удаляемое» с учётом каскада FK, имена людей в JSON (хроника, уведомления, плашки чатов) только парой с id (GIN), томбстоун-маркер, канарейка стирания |
| [lifecycle_export.md](lifecycle_export.md) | Выгрузки данных целиком (`core/lifecycle`): архив человека и организации (окно SMS `data_export`, раз в сутки, ZIP-части, 5 выдач, 5-минутные ссылки, поля видимости глазами заказчика, перепроверка владельца), провайдеры модулей, восстановление одной организации из бэкапа (подписанный архив, импорт с теми же id, реплей стираний) |
| [operations_backup_dr.md](operations_backup_dr.md) | Рунбук: обещания RPO/окна PITR, что копируется, восстановление всей базы (сверки, реплей журнала стираний), восстановление одной организации, учения, резидентность побочных каналов |
| [lifecycle_settings.md](lifecycle_settings.md) | Сроки организаций (коридор пол закона ↔ потолок тарифа, сокращение через 30 дней с уведомлением и отменой, предпросмотр), принуждение при чтении (хроника, доставки, пол `seq` чата), таймер автоудаления чата, чип «Заморожено» карточки записи, дашборд «Данные» Кабинета (6 вкладок, команды, панели 360, отчёты бэкапов с подписью) |
