...
- **Обслуживание — единственный крон движка** (`JobsCron`, каждый под Redis-локом):
  ...
  - ретеншн `20 4 * * *` — `pruneTerminal`: completed — сутки (политика `Job` реестра `core/lifecycle`, отсчёт от `finished_at`) / discarded+cancelled 30д, батчами `RETENTION_BATCH` (5000);
