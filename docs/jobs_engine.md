...
- **Обслуживание — единственный крон движка** (`JobsCron`, каждый под Redis-локом):
  ...
  - ретеншн — шаг `jobs.terminal` раннера сроков core/lifecycle (`pruneTerminalBatch` — одна пачка ≤ limit): completed — сутки, discarded+cancelled — 30 дней; сроки — политика `Job` реестра (основное правило + `extraRules`), отсчёт от `finished_at`; дев-ручка `POST /jobs/dev/prune` — весь хвост сразу (`pruneTerminal`);
  - перестройка индексов `40 4 * * 0` (Алматы) — `reindexQueue` через обслуживающее подключение `DatabaseMaintenance` (прямой адрес `DIRECT_URL` мимо PgBouncer, потолки роли приложения сняты — перестройка длится минуты): `REINDEX TABLE CONCURRENTLY jobs` (горячая таблица с постоянными UPDATE статуса и DELETE ретеншна раздувает btree; CONCURRENTLY не блокирует запись); сперва удаляются невалидные дубли `*_ccnew*` оборванного прошлого прогона (они пишутся при каждой вставке, но не читаются). Хранение таблицы — `fillfactor=80` (HOT-обновления статуса), автовакуум по порогу 1000 строк, а не по доле (миграция `core_lifecycle`).
- **Тип без обработчика** (фича выключена env / тип удалён между деплоями): строка бессмертна (claim, фиксап — по реестру; ретеншн — только терминальные). Движок видит и называет (`stats.unhandled` + часовой warn), НЕ пускает в прибор «очередь встала»; авто-чистки НЕТ намеренно. Хоронит человек: `purgeUnhandled(type)` → `cancelled` + finishedAt (заберёт ретеншн); только `available` (executing может исполнять сосед); отказ `409 dev.jobTypeRegistered`, если тип ЗАРЕГИСТРИРОВАН на этом инстансе.
...
- `analytics` (2): `analytics.rollup.day` (день целиком под advisory-локом; `uniqueKey rollup:<день>`, ставит крон по «грязным» дням) · `analytics.report.run` (воронка > 90 дней; `uniqueKey report:<hash>`) · `analytics.user.erase` / `analytics.workspace.erase` (два прохода, второй `runAt` +10 минут) — `apps/api/src/core/analytics/analytics.constants.ts`.
...
## Ключ «ровно один раз» вне HTTP

У джоба нет заголовка `Idempotency-Key`, но есть `uniqueKey` — и он обязан быть ДЕТЕРМИНИРОВАННЫМ. Собирать его руками из строк опасно (легко подмешать время или случайное число, и защита молча исчезнет), поэтому есть `IdempotencyService.stableKey(namespace, …parts)` (`core/idempotency`): одинаковый вход ⇒ одинаковый ключ (приём Shopify UUIDv5). Части — только стабильные: id сущности, ключ периода, номер попытки домена.

Пакеты планировщика («начислить за март») дедуплицируются «входящим ящиком» того же движка: `IdempotencyInboxService.once(tx, { source: 'scheduler', account, eventId: periodKey })` в транзакции постановки. Урок Santander: без ключа периода повторный прогон планировщика рассылает пакет второй раз.
