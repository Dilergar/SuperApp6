...
- Отказ по области (`OrgRightsService.forbid`) принимает КЛЮЧ каталога, а машинный код (`org_scope_forbidden`) у всех его причин один: клиент ветвится по коду, а фраза меняется — «не ваша ветка» и «нужен Менеджер» это разные тексты одного кода.
