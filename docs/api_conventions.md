...
- **Сырой SQL**: Prisma шлёт строку как text — сравнение с uuid-колонкой только с приведением `${id}::uuid` / `::uuid[]`; `LIKE` и склейка по id запрещены; агрегата `max()/min()` у uuid в PostgreSQL нет (`max(col::text)`); `COALESCE(uuid, '')` невозможен — уникальность с NULL выражается `NULLS NOT DISTINCT`. Аудит — `node apps/api/scripts/audit-raw-sql-uuid.cjs` (и `--dir scripts` для сьютов): находок 0.
