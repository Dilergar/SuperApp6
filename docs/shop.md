...
- **Магазин организации в её каскаде удаления** (`shop.owner`, `ShopService.closeOwnerShop`): живые заказы (pending/funding/confirmed) закрываются с возвратом денег — эскроу снимается целиком (у складчины — всем), покупатель получает «отклонён»; витрины с лотами, их права и фото уходят; история заказов остаётся (срок закона, `listingId` обнуляется).
