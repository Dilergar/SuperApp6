...
- **Потолок тарифа считает ЖИВЫЕ узлы, поэтому возврат из архива проверяет его как создание.** Провайдер расхода `objects.maxPerWorkspace` (`core/entitlements`) считает `archivedAt: null` — иначе шкала «1998 из 2000» состояла бы из узлов, которых нет ни в дереве, ни в выборках. Обратная сторона: `restore` вызывает `assertCanCreate` на ВСЁ оживающее поддерево (`402 entitlement.limit_reached`), иначе архив был бы обходом тарифа — закрыть 2000, создать 2000, вернуть закрытые.
