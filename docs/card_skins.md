...
- **Имя и описание скина — платформенный КОНТЕНТ, а не данные владельца**: сид кладёт в БД КЛЮЧ каталога (`profile.skins.catalog.<slug>.name|description`), а слово собирает сервис в языке запроса (`CardSkinsService.phrase`). Значение, которого каталог не знает (скин, заведённый руками), печатается как есть — так же читаются и старые строки. Сьюта ищет сид-скины по фиксированным id, а не по имени ([i18n.md](i18n.md)).
