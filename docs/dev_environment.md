...
# 1. Инфраструктура (PostgreSQL 18 + PgBouncer 1.26 + Redis 8.10 ×2 — версии пинованы в docker-compose.yml и CI)
...
- **Redis-состояние** (`superapp6-redis`, `:6379`, `infra/redis/state.conf`: noeviction + AOF) и **Redis-кэш** (`superapp6-redis-cache`, `:6380`, `cache.conf`: allkeys-lfu без персистентности). ACL — `infra/redis/users.dev.acl`: приложение входит `sa6_app` с прод-правами — перечнем команд (команда вне перечня → `NOPERM` в dev до деплоя; новая команда в коде → строка в `users.dev.acl` И `users.acl.template`), `default` без пароля оставлен сторонним dev-компонентам (LiveKit) и ручной отладке `redis-cli`. После регресса — `node apps/api/scripts/redis-acl-denials.cjs` (отказы `sa6_app` в ACL LOG обоих инстансов; `--reset` — очистить перед прогоном). Модули образа не грузятся (пустой tmpfs в compose). `protected-mode no` — только флагом compose (dev-`default` без пароля); прод-конфиг держит `yes`. Формат ACL-файла комментариев не допускает — пояснения в `state.conf`.
- **Обновление версии Redis** (тег в `docker-compose.yml` — оба инстанса): сначала копия тома состояния — обратно данные новой мажорной версии старая не прочтёт: `docker exec superapp6-redis redis-cli BGREWRITEAOF` (дождаться `aof_rewrite_in_progress:0`), `docker compose stop redis`, `docker run --rm -v superapp6_redis_data:/from:ro -v superapp6_redis_backup_<версия>:/to alpine cp -a /from/. /to/`; затем `docker compose up -d redis redis-cache` и сверка ключей (`DBSIZE`, выборочно типы и значения). Кэш-инстанс без персистентности — после рестарта пуст по замыслу (читатели пересоберут из базы).
