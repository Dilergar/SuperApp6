...
- **Telegram** (`POST /processes/webhook/telegram/:token`) — `@SkipIdempotency('inbound_webhook')`: дедуп по `update_id` через «входящий ящик» движка идемпотентности, и отметка ставится ТОЛЬКО после проверки `secret_token`. Telegram повторяет доставку, пока не увидит 2xx, — без дедупа один апдейт запускал бы процесс столько раз, сколько было повторов. Запуск живёт не в одной транзакции с отметкой, поэтому при его отказе отметка снимается (`forget`).
- **Общий вебхук-триггер** (`POST /processes/webhook/:token`) — наоборот, работает по ОБЫЧНОМУ ключу повтора: у произвольной внешней системы нет ни подписи, ни своего идентификатора события, и единственная защита, которую мы можем ей дать, — заголовок `Idempotency-Key`. Принципал — хэш токена триггера, поэтому чужой ключ в чужом триггере остаётся «невиданным». **Без заголовка дедуп невозможен — это правило для интеграторов.**
