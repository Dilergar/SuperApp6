...
```
processes.node.<тип>.title | .description
processes.node.<тип>.output.<ключ> | .input.<ключ>          ← иначе общее processes.output.<ключ>
processes.node.<тип>.field.<ключ>.label | .help | .placeholder  ← иначе общее processes.field.<ключ>.*
processes.node.<тип>.field.<ключ>.option.<значение>       ← иначе processes.field.<ключ>.option.<…>,
                                                            иначе общий словарь processes.option.<значение>
```

Своё слово ноды перевешивает общее — поэтому `credentialId`, `text`, `model` описаны один раз на всю палитру, а «часов», «≥» и имена событий — один раз в словаре ЗНАЧЕНИЙ (`processes.option.*`), сколько бы полей их ни предлагало. Новая нода = паспорт + ветка каталога; `pnpm check:i18n` держит паритет трёх языков. Сообщения Zod-схем самих нод — тоже КЛЮЧИ (`processes.validation.*`), их разворачивает компилятор через `registry.text()`.

## Версии и компилятор

Модель Salesforce Flow: publish = новая версия, активна одна; правка published авто-открывает черновик v+1; бегущие доживают на своей версии (pin). Мягкая валидация при сохранении; публикация — 0 issues + исполнители-члены; `ProcessValidationIssue.severity` (`error` блокирует; `warning` — публикация с ПОИМЁННЫМ принятием `acceptWarnings`, след — запись `process.published_with_warnings` в хронике организации со списком правил; отсутствие поля = error, fail-closed). Правила ТК РК для кадровых маршрутов — `process-document-rules.ts` (предупреждениями). Замечания компилятора и правил несут КЛЮЧ каталога с подстановками (`RawIssue`), а слова добавляет `ProcessesService.renderIssues` перед выдачей наружу — компилятор чистая функция и переводчика не имеет. Недозаполненная нода остаётся в плане с сырым конфигом (чтение страницы не падает; за ней схема достижима); self-loop и рёбра в триггер запрещены.
...
**Язык рантайма.** Подпись шага в скомпилированном плане (`n.label || <название типа>`) и текст ошибки шага ЛОЖАТСЯ в БД, поэтому пишутся в языке ИСТОЧНИКА (`registry.sourceTitle`, английские литералы) — как всякий снимок. Пустая подпись ноды — норма: канвас подставит название типа в языке зрителя. Промпты и описания инструментов агента остаются английскими: их читает МОДЕЛЬ, а не человек.
