...
- **Подпись узла может принадлежать платформе**: маршрут, собранный из библиотеки бланков, несёт у узла `labelKey` (+`labelParams`) — подпись собирается при ЧТЕНИИ в языке зрителя, а `label` остаётся снимком языка-источника. Как только человек переписал подпись, ключ гаснет (`dropTouchedLabelKeys`) и текст становится текстом организации ([i18n.md](i18n.md)). Тем же приёмом едет заголовок шага в `core/approvals` (`titleKey`/`titleParams`).
...
**Контракт запроса к Claude** (`process-ai-client.ts`). `temperature` Claude не уходит никогда (текущие модели отвечают 400) — поле видно только openai-провайдерам; у Claude его место занимает `effort` (`output_config.effort`, пусто = умолчание модели). Размышления у текущих Claude включены по умолчанию и входят в `max_tokens`: умолчание 16000, потолок 16000 (без стриминга). `stop_reason` `refusal`/`max_tokens` — ветка error, а не «успех» с пустым текстом; обрезанный `tool_use` не исполняется. Парсер у Claude — structured outputs (`output_config.format` со схемой из полей парсера), у openai-провайдеров — инструкция текстом + вырезание JSON. Память агента уходит модели ходами диалога перед текущим, НЕ в `system`: прошлый ввод людей не получает полномочий оператора. Отказ инструмента — `tool_result` с `is_error` (исполнитель бросает, а не возвращает строку). Имена инструментов в запросе уникальны (повтор — суффикс `_2`; подпись по-русски → `agent_<id>`), у ноды-инструмента в описании её подпись. Прогон агента со всеми под-агентами укладывается в общий дедлайн 170 с < аренды I/O-шага 200 с (иначе крон переисполнит шаг и отправки задвоятся). Выход AI-нод несёт `usage` (`inputTokens`/`outputTokens`) — учёт стоимости.
...
## Триггеры и повторная доставка

- **Telegram** (`POST /processes/webhook/telegram/:token`) — `@SkipIdempotency('inbound_webhook')`: дедуп по `update_id` через «входящий ящик» движка идемпотентности, и отметка ставится ТОЛЬКО после проверки `secret_token`. Telegram повторяет доставку, пока не увидит 2xx, — без дедупа один апдейт запускал бы процесс столько раз, сколько было повторов. Запуск живёт не в одной транзакции с отметкой, поэтому ящик работает с арендой (`begin` → `done` | `forget`): отказ запуска снимает отметку, смерть процесса посреди запуска — аренда истекает, и редоставка забирает апдейт. Редоставка, пришедшая, пока первая ещё в работе, получает `409` (единственный не-2xx этой ручки): на 200 Telegram счёл бы апдейт принятым.
- **Общий вебхук-триггер** (`POST /processes/webhook/:token`) — наоборот, работает по ОБЫЧНОМУ ключу повтора: у произвольной внешней системы нет ни подписи, ни своего идентификатора события, и единственная защита, которую мы можем ей дать, — заголовок `Idempotency-Key`. Принципал — хэш токена триггера, поэтому чужой ключ в чужом триггере остаётся «невиданным». Повтор отдаёт сохранённый ответ, не вызывая ручку, поэтому у неё есть **шлюз повтора** `processes.webhookTrigger` (`liveWebhookTrigger` — та же проверка, что и у запуска): триггер выключили или удалили — повтор слышит тот же 404, а не «запущено» из снимка. **Без заголовка дедуп невозможен — это правило для интеграторов.**
