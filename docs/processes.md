...
**Человеческие**: Задача человеку (создаёт НАСТОЯЩУЮ задачу; исполнитель сотрудник|отдел(очередь+claim, модель Camunda candidate-group)|инициатор; dueInHours→SLA+эскалация) · Решение человека (`human.approval` → движок core/approvals: kind согласование|подпись|ознакомление, адресат человек/должность/отдел/филиал, rule any|all, исход «На доработку»; возврат токена — хук ApprovalOriginRegistry → `resumeApproval`; причина отказа обязательна В ДВИЖКЕ).
