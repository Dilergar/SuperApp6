...
> Вторая часть [module_graph.md](module_graph.md) — синхронные рёбра Диска, офисных документов, гостевых ссылок, Документооборота, Контрагентов, КЭДО и движков подписи/согласований/шаблонов. Правила связей, DI-токены и carve-outs — в первой части; шина — [event_bus.md](event_bus.md).
...
- `ApprovalsService` → `AccessService` (`principalsOf` — снимок адресатов при активации), `JobsService`; `ApprovalsRichCardProvider` → `RichCardRegistry`.
