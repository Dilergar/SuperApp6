...
- **Сроки и таймер** ([lifecycle_settings.md](lifecycle_settings.md)): таймер автоудаления (`Chat.messageTtlDays` 1·7·30 или выкл; личный чат — любой участник, группа — владелец/админ, контекстный — нет; в чате организации не длиннее её срока) и срок сообщений организации действуют СРАЗУ при чтении — пол `seq` чата (`MessengerRetentionService`; ищется двоичным поиском по индексу `(chat_id, seq)` — у чата под заморозкой истёкшие сообщения живут, и скан с головы чата платил бы за них каждые 5 минут) режет ленту, непрочитанные, превью, цитату (и в ответе правки, и в сокете `message.updated` — цитата старше пола рисуется удалённой), поиск, вложения и действия по id; снимок текста для уведомлений (упоминание, «отложенное отправлено») не пишется, если сообщение истечёт раньше уведомления (`snippetAllowed`); отложенное с истёкшей или удалённой к отправке цитатой уходит без неё (при постановке — `404 chat.messageNotFound`); правка — status-guarded (томбстоун текст не получает). Раннер ночью лишь освобождает место.
