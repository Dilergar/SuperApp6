...
- **Сроки и таймер** ([lifecycle_settings.md](lifecycle_settings.md)): таймер автоудаления (`Chat.messageTtlDays` 1·7·30 или выкл; личный чат — любой участник, группа — владелец/админ, контекстный — нет; в чате организации не длиннее её срока) и срок сообщений организации действуют СРАЗУ при чтении — пол `seq` чата (`MessengerRetentionService`) режет ленту, непрочитанные, превью, цитату, поиск, вложения и действия по id; раннер ночью лишь освобождает место.
- **Стирание человека** (шаг `messenger.subject`, [lifecycle_erasure.md](lifecycle_erasure.md)): имена в системных плашках о нём — меткой томбстоуна (плашки находит индекс `message_person_ids`, снимок текста пересобирается); по выбору в мастере — все его сообщения томбстоунами; личный чат остаётся собеседнику (строка участника — ссылка на томбстоун), а когда стёрты ВСЕ собеседники (или это «Избранное» с самим собой) — удаляется целиком; группа — выход с плашкой (владение — старейшему админу, иначе участнику, без наследника — удаление); контекстный чат — выход без плашки.
...
- **Плашку сервиса (офис, звонки, офисный документ, события календаря, заказы) ставят через `SystemPlaque`**, а не готовым текстом: продюсер называет `typeKey` записи хроники и её значения, `postPlaque` пишет снимок источника и кладёт `chatterTypeKey` в payload. Имя СОБЫТИЯ шины (`office.room.created`) не совпадает с ключом типа (`office.room_created`) и остаётся в `eventType` — по нему ветвятся клиенты. Готовая строка в аргументе замораживала бы плашку в языке того, кто её поставил (и прятала бы имя от стирания).
- **Имя человека в плашке — только парой с id**: актор — `values.actorName` + `SystemPlaque.actorId` (в структуре — `chatter.actorName` ↔ `chatter.actorId`), цель — `targetName` ↔ `targetUserId` (`PERSON_NAME_REFS`); нет имени — пусто («кто-то») или ключ (`targetNameKey: 'messenger.participantFallback'`). `postSystemMessage` проверяет пары (`plaquePersonProblems`): в деве — ошибка, в проде — журнал. Иначе имя стёртого человека переживёт его в чужих чатах.
