...
- Конец жизни один — `archive` (кнопка владельца, осиротевший файл, каскад организации): статус `archived` + бамп `tokenEpoch` + **`deletedAt` = момент закрытия**. От `deletedAt` считает срок шаг `docs.trash` раннера сроков core/lifecycle (`DOCS_LIMITS.archivedPurgeDays`, 30 дней; политика `Document`): строка и неподписанные вехи уходят, документ с ПОДПИСАННОЙ вехой — никогда (на неё ссылается ЭЦП). Каскад организации — хук `docs.owned`: `archive` каждого её документа, неподписанные уходят сразу (окно восстановления уже было сроком архива организации); заморозка удерживает на обоих путях.
