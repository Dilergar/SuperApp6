...
- **Методы `system*`/служебные прав НЕ проверяют** — по контракту это делает вызывающий (первое, что смотрит ревью нового потребителя): `linkSystemInTx`, `linkManyInTx`, `countLinkedInTx`, `unlinkSystem`, `unlinkAllForRef`/`unlinkAllForRefs`, `systemDeleteFile`, `systemDeleteAllOwnedBy(ownerType, ownerId, { deadline })` (хук `files.owned` каскада удаления организации: все файлы владельца тем же системным путём, с бюджетом — доказательства и файлы под `blocksDeletion` пропускаются; затем хук сразу удаляет их физически — байты и строка каждого в транзакции с проверкой заморозки, без 7 дней корзины: восстанавливать некому), `createCanaryFile` (только посевы канарейки стирания: готовая строка + байты + квота владельцу, без конвейера и событий `file.*`), `deletionGuardedRefTypes`, `ingestLocalFile` (headless-инжест), `replaceContent`, `copyFile`, `putDerivedVariant`, `buildSystemDownloadUrl(fileId, variantKind?)`, `openKeyStream`, `openRawStream`, `localPathFor`, `listLinked` («доступ гейтит вызывающий»), `getLinkedFileIds`, `listLinksOfFile`, `getUsageFor`, `sweepOrphanReady`. Проверяют права: `getMeta(viewerId, …)`, `canViewFile`, `canEditContentVia`, `unlinkAndReap(actorId, …)` и все HTTP-ручки.
...
- Жизненный цикл `FilesCron` (`files.cron.ts`): брошенные `uploading` > 24ч (`7 * * * *`) · **сироты `ready` без настоящих мест** (`23 * * * *`, `sweepOrphanReady`, грейс 24ч, только приватные — публичные живут без FileLink) · сверка квот (`40 4 * * *`). Сроки — шаги раннера core/lifecycle (`files.lifecycle.provider.ts`): `files.deleted` — физическое удаление soft-deleted через 7 дней (политика `FileObject`; байты и строка — в транзакции с проверкой заморозки) и профили со своим сроком (`blob:audit_export` — выгрузки журнала безопасности 7 дней → системное удаление); `files.upload-tmp` — временные каталоги старше суток. Стирание человека — шаг `files.subject` ([lifecycle_erasure.md](lifecycle_erasure.md)): личные файлы без живых ссылок — системное удаление и сразу физически (тот же хвост, что у каскада организации). `unlinkAndReap` реапит только когда связь реально снята. События: `file.deleted`.
- **Временные файлы платформы — только в своих каталогах** (`shared/fs/temp-file.util.ts`): `appTmpDir()` (`<tmp>/superapp6`, права 0700: выгрузки, перекодирование медиа, PDF, multer STT) и `storageTmpDir()` (`<FILES_LOCAL_ROOT>/tmp`: части загрузок multer, тело WOPI, восстановление версий — один том с хранилищем, rename дёшев); путь — `appTmpPath(name)`, байты с гарантированной уборкой — `withTempFile`. Голый `os.tmpdir()` не используется: уборка по сроку не должна задевать чужое в /tmp, а брошенная загрузка (обрыв клиента, multer CVE-2026-88932) иначе живёт вечно.

## Выгрузки журнала безопасности

Профиль `audit_export` (документ, до 200 МБ, `application/x-ndjson` | `text/csv`, приватный): файл собирает джоб журнала и заводит `ingestLocalFile`; ссылки `audit_export` (право — owner/admin организации) и `audit_platform_export` (право — только сотрудник-автор, в продукте не виден никому) регистрирует `core/audit` в `FilesRefRegistry`. Модуль экспортирует `STORAGE_DRIVER` — им журнал пишет архив месяцев и копии дайджестов в то же хранилище (`audit/archive/`, `audit/digests/`) — [audit_engine.md](audit_engine.md).

## Антивирус (ClamAV, джоб `files.scan`, СВОЯ очередь scan, cap 3)

Включается `CLAMAV_HOST` (+ `CLAMAV_PORT`, по умолчанию 3310; иначе no-op, scanStatus='none'). Политика `shouldScanFile` (shared): «по умолчанию проверяем, исключения явные» — НЕ проверяются картинки/аудио/видео/обычный офис; проверяются архивы, PDF, текст, макро-офис (.docm и т.п.), всё неопознанное; статус `skipped` честный. clamd INSTREAM (TCP, сокет-таймаут 120с — аренда джоба ОБЯЗАНА быть больше); infected → блок выдачи; вердикт и событие журнала `files.malware_detected` — одной транзакцией (переход статуса по условию), уведомление загрузившему — паспортом события ([audit_engine.md](audit_engine.md)), серия заражённых загрузок — детекция `malware_burst`; пустой ответ clamd — транзиент; метка pending ставится в транзакции постановки; висящие `pending` подхватываются на старте. `uniqueKey` джоба — `fs:<fileId>`.
