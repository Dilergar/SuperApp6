...
## Push-уведомления Google: токен канала

Приёмник `POST /integrations/google/webhook` аутентифицирует отправителя заголовком **`X-Goog-Channel-Token`**: `registerWatch` кладёт в канал HMAC keystore от id канала (имя ключа `google_channel`), приёмник сверяет его константным временем. Без этой сверки приёмник верил бы одному лишь id канала из заголовка — то есть был бы открыт.

Каналы, зарегистрированные до секрета, несли id человека. Такой токен принимается ОДИН раз (тоже константным сравнением) и тут же перерегистрирует канал с настоящим секретом — окно наследия закрывается первым же уведомлением.

Ящика ровно-одного-раза здесь нет намеренно: синхронизация инкрементальна по `syncToken` Google и естественно идемпотентна — повтор просто не находит нового. По той же причине `POST /integrations/google/sync` объявлен `@SkipIdempotency('naturally_idempotent')`.
