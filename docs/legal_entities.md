...
- **Самолечение** `ensureHeadLegalEntity(ws)` по образцу `ensureDefaultBranch`: организация без строки чинится первым же обращением; новая организация получает головное юрлицо в ТОЙ ЖЕ транзакции, что и себя. Новое юрлицо (и головное, и обычное) получает основание подписи по умолчанию СТРУКТУРОЙ (`kind: 'ustav'`), а печатное «Устава» рождается на выходе — в языке зрителя на экране и в языке БЛАНКА внутри договора ([counterparties.md](counterparties.md)); клиент его не сочиняет.
...
- **Потолок тарифа считает ЖИВЫЕ юрлица, поэтому возврат из архива проверяет его как создание.** Провайдер расхода `legalEntities.maxPerWorkspace` (`core/entitlements`) считает `archivedAt: null`; `restore` идёт транзакцией с `assertCanCreate` и отвечает `402 entitlement.limit_reached`, если живых уже столько, сколько даёт ступень. Иначе архив был бы обходом тарифа.
