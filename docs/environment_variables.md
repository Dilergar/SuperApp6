...
- `GEO_COUNTRY_HEADER` — имя гео-заголовка страны, который ставит край сети и затирает у клиента (за Cloudflare — `cf-ipcountry`). Страна журнала безопасности (`core/audit`), новизна «вход из новой страны», «откуда» ключа API — только из него. Пусто → страны нет. Список известных CDN-заголовков (`countryFromHeaders`) годится лишь для догадки о языке: его подделывает клиент
...
- `LIFECYCLE_RESTORE_SOURCE_URL` — кластер, поднятый из бэкапа на точку времени (PITR), роль только на чтение: источник команды восстановления одной организации `lifecycle.restore.extract`. Пусто → в production команды восстановления отказывают `lifecycle.restoreSourceMissing`, в development источником служит основная база — [lifecycle_export.md](lifecycle_export.md), [operations_backup_dr.md](operations_backup_dr.md)
- `LIFECYCLE_OPS_TOKEN` — токен скриптов эксплуатации (pgBackRest, restore-drill) для `POST /lifecycle/ops/backups/report` (≥ 32 символов): Bearer + ключ HMAC-подписи тела (`X-Lifecycle-Signature: t=<unix>,v1=<hex>`, окно 300 с); пусто → в production 404, в development открыт — [lifecycle_settings.md](lifecycle_settings.md)
...
## Журнал безопасности (`core/audit`) — [audit_engine.md](audit_engine.md)

- `AUDIT_DIGEST_INTERVAL_MIN` — интервал подписанных дайджестов целостности, минут (пусто → 5; 1…60)
- `AUDIT_RETENTION_YEARS` — срок хранения месяца в PostgreSQL, лет (пусто → 3; 3…25). Ниже 3 не бывает: пол держит и функция сброса `audit_drop_partition` в самой базе (ЕТ № 832 п. 38)
- `AUDIT_ARCHIVE_ENABLED` — выгрузка закрытых месяцев в объектное хранилище (NDJSON+gzip + подписанный манифест); пусто → включено, `false` — аварийный стоп (месяц без архива не сбрасывается НИКОГДА). Хранилище — драйвер `core/files` (`FILES_DRIVER`/S3), префиксы `audit/archive/` и `audit/digests/`
