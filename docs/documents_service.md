...
- **`DocTemplate`** — бланк: `kind` `'docx'` (файл core/docs) | `'builder'` (блоки `builderDoc` — конструктор) + `fields` (форма подачи; kind `daterange` — период дат) + `selfService`; draft|published. **Доступность** — гранты `doc_template#requester` (человек/отдел/должность/филиал; с `subjectRelation`!), читаются `grantSetFor` одним условием; личная Группа отвергнута (шаблон живёт в организации). Выдача и снятие — в транзакции с событием журнала `sharing.access.granted|revoked` (`access: use`).
...
- **Слова живут в каталоге, синтаксис бланка — нет.** Подписи статусов, категорий, видимости и
  полей — ключи неймспейса `documents`; имена тегов («{Document.Number}», «{Form.Vacation Days}»),
  плейсхолдеры формата номера («{YYYY}») и имена форматов чипа («date:long») — это DSL БЛАНКА:
  английские идентификаторы (модель Salesforce: API-имя одно на все языки бумаги), они живут в
  `shared/constants/doc-template-dsl.ts`, а подпись к ним даёт каталог.
- **У бумаги свой ЯЗЫК.** `DocTemplate.language` (умолчание — `Workspace.documentLanguage`, по
  рынку казахский) и СНИМОК `OrgDocument.language`: перевод бланка завтра не перерисовывает
  поданное вчера. В этом языке печатаются слова платформы внутри документа — юрформа, налоговый
  режим, основание подписи, «М.П.», «№ … от …», сумма прописью — и в нём же уходит SMS
  контрагенту со ссылкой на подписание ([i18n.md](i18n.md)).
