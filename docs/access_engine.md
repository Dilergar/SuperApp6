...
- Боты `core/keys` — НЕ новый тип принципала: бот = строка `users` (`kind='bot'`) с ролью организации в `user_roles`, поэтому проекция ролей, `check()` и `grantSetFor` видят его как обычного `user`. Скоупы ключа сужают доступ ДО движка (`KeyScopeGuard`, по сервису маршрута), объектное право дальше решает движок — [keys_api_access.md](keys_api_access.md).

- Видимость ПОЛЕЙ видимой записи (карточка человека, реквизиты сотрудника, оклад, контакты контрагента) — не движок прав, а `core/visibility` поверх него: [visibility_engine.md](visibility_engine.md). Способности `card.view_full` больше нет; кто видит реквизиты сотрудника, решает политика типа `staff.member`.
