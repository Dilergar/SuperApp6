...
- **Учёт действий** (Правила № 179/НҚ п. 9 пп. 5) — события `pd.transfer | pd.cross_border | pd.publication` журнала безопасности ([audit_engine.md](audit_engine.md)); `fields` — КОДЫ полей, никогда значения. «Срок согласия» (`consent_term`) — это события `consents.accepted | consents.revoked`, которые пишет сам движок при приёмке и отзыве (один факт — одно событие). Категории `pd` и `consents` окно ленты человека не режет: «Мои данные» показывают весь учёт.
- **`PdIncident`** + **`PdIncidentEvent`** (append-only).
- `VerifyChallenge.context` — что человек принял ДО отправки SMS; `User.consentEpoch`.

**Неизменяемость — на уровне базы** (миграция `core_consents`): триггер `consent_versions_guard` (у не-черновика меняются только статус, подпись, заверение, отметка активации; DELETE — только черновик; **статус ходит только вперёд**: `published → superseded | withdrawn`, оба конечны, а `withdrawn` доступен лишь не вступившей версии — подпись статус не покрывает, и без лестницы одна правка колонки «оживляла» бы заменённый или отозванный текст, а отзыв вступившей существенной версии уменьшал бы G), `consent_acceptances_guard` (DELETE запрещён, UPDATE — только однократный отзыв и перешивка шифротекста), append-only у событий инцидентов (учёт действий держит append-only журнала безопасности). IP и User-Agent приёмки шифруются ПЛАТФОРМЕННЫМ ключом `core/keys`, не KEK человека: крипто-шреддинг аккаунта не должен уничтожать доказательство.
...
- **Служебный отзыв (`system: true`) хуки не зовёт** — им пользуется тот, кто сам уже погасил данные (кнопка «Отключить Google»); удаление аккаунта — исключение. Мастер «Это не я» отзывает НЕ служебно, с причиной `not_me`: иначе согласие гасло бы, а подключение с токенами жило.
...
- `record(null, …)` — best-effort для мест без транзакции (канал уже отправил сообщение; `recordBestEffort` журнала); с `tx` — атомарно, в транзакции факта. Партиции журнала заводит функция владельца заранее, поэтому момент события в транзакции всегда «сейчас».
...
1. `node apps/api/scripts/consents-publish-initial.cjs` — план (ничего не меняет); `--publish` — публикация; `--receipt=<файл.json>` — копия распечатки хэшей вне базы (существующий файл не перезаписывается, папка проверяется ДО публикации). Свойства, на которых держится доверие к скрипту:
   - **запирает сам себя**: есть хотя бы один человек (считаются и удалённые) — отказ «публикуйте из Кабинета»; публикуются только документы БЕЗ единой опубликованной версии, живой документ скрипт не трогает никогда;
   - **правда — файлы** `apps/api/consents-texts/<документ>.<язык>.md`. Засев создаёт черновик только документу без версий, поэтому файл, поправленный после первого старта API, в базу сам не попадает: скрипт сверяет черновик с файлами, помечает расхождение в плане и перед публикацией переписывает черновик из файлов (в журнале — `draftRewrittenFromFiles`). Документ без файлов-исходников роняет запуск: публиковать «что нашлось в базе» нельзя;
   - **одна транзакция на весь пакет**: сбой на любом документе откатывает всё — платформа не запускается наполовину (регистрация открыта, условий для организаций нет). Сообщение о сбое честно различает «ничего не опубликовано» и «опубликовано, сломалась только распечатка»;
   - публикация идёт движком (хэши, цепочка манифестов, подпись платформы, триггеры), распечатка читается через `loadVerified`; в журнал Кабинета — строка на документ `consents.document.bootstrap_publish` с `actorId = null` (как у bootstrap владельца);
   - перед бутом зовётся `validateEnv()` — как в `main.ts`; коды выхода: 0 — план/успех/нечего публиковать, 1 — отказ или сбой, 2 — неверный вызов.
...
Правки юриста обязаны лежать в файлах `consents-texts/` ДО шага 1: публикуется ровно то, что в них написано (план показывает размер каждого текста по языкам — сверка глазами перед необратимым шагом). После публикации единственный источник текста — база. Работающему API рестарт не нужен — микрокэш версий живёт 2 секунды. Заверение ЭЦП руководителя (`consents.document.attest`) необязательно и делается позже из Кабинета.

**Учение запуска** ставится на ОТДЕЛЬНОЙ базе (`CREATE DATABASE` → `prisma migrate deploy` с её `DATABASE_URL` → скрипт с `NODE_ENV=production`), никогда на рабочей: проверяются план, устаревший черновик (подменить `bodies` черновика — план обязан пометить расхождение, а опубликоваться обязан текст файла), атомарность (триггер-сбой на последнем документе → 0 опубликованных и 0 строк журнала), квитанция и отказ после первого человека.
