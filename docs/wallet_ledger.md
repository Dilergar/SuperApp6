...
`deriveKey` считается из СТАБИЛЬНЫХ входов (скоуп запроса + ключ клиента + имя шага), а не из id строки заявки: строка умирает через неделю, и ключ от её id сменился бы — ремень отвалился бы ровно на повторе через неделю. `LedgerTransfer.idempotencyKey` уникален; `mint`/`burn`/`transfer` проверяют дубль ДО вставки и при находке не трогают остатки вовсе. Дубль сверяется по параметрам (валюта, сумма, счета, вид `posted`): тот же ключ с ДРУГИМИ параметрами — не повтор, а переиспользованный ключ → `409 wallet.idempotencyKeyReused` (тихое «уже сделано» соврало бы вызывающему, что деньги ушли). Ключ живёт вечно вместе с проводкой (леджер append-only) — ключ-литерал в сьюте обязан быть своим на прогон.
