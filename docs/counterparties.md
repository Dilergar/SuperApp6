...
## Видимость контактов (`counterparty`)

Реквизиты контрагента (название, БИН, адреса, КБе, налоговый режим, директор, основание подписи) — пол типа `counterparty`: их видит каждый, кому открыт справочник. Телефон, e-mail, контакты и IBAN — служебные поля ([visibility_engine.md](visibility_engine.md)): владельцу, админу и менеджеру — целиком, сотруднику и стажёру — маской (`+7 70* *** *5 67`, `s•••@mail.kz`, последние четыре IBAN), подрядчику — скрыто; организация меняет это матрицей. Список, карточка, рич-карта и отправка документа контрагенту (`SendToCounterpartyModal`) рисуют `<GuardedValue>`; правка маски в теле → `400`, правка невидимого → `403`. Хроника контрагента объявляет `visibility`: смена телефона стажёру — «Скрыто».
