...
- Типы — dot-namespaced (`contact.invitation.received`, `task.assigned`…), реестр `NOTIFICATION_REGISTRY` (shared) несёт СМЫСЛ типа: иконку, категорию настроек, флаг pushByDefault (push-доставки пока НЕТ — mobile-этап). СЛОВА живут в каталоге `@superapp/i18n`: `notifications.<type>.title` и (необязательно) `.body` — [i18n.md](i18n.md).
- **Два пути создания**:
...
- **Язык.** `notify()` пишет `title/body` в языке АДРЕСАТА (`User.locale`) — это фолбэк для push и старых клиентов; фанаут джоба берёт языки ПАЧКОЙ (`I18nService.localesFor`), а не по разу на человека. `list()` ПЕРЕРИСОВЫВАЕТ текст из `type + payload` в языке ЗАПРОСА: иначе смена языка не трогала бы накопленную ленту. Тип, которого больше нет в реестре, показывает запечённый текст.
- Правило нового сервиса: тип в реестр + ветка в карту + три файла каталога (`en`/`kk`/`ru`); слать `emitEvent`, НЕ голым `events.emit`.
