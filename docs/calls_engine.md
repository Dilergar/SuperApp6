...
Обработка живёт не в одной транзакции с отметкой (внутри есть вызовы LiveKit — `deleteRoom`), поэтому используется аренда ящика: `begin()` → обработка → `done()` | `forget()`. Упала обработка — отметка снимается, и редоставка пройдёт заново. Умер процесс посреди обработки (`forget` позвать некому) — аренда истекает, и редоставка забирает событие (`handleWebhook` идемпотентен). Редоставка, пришедшая, пока первая доставка ещё В РАБОТЕ, получает **`409 idempotency.in_flight`**, а не 200: услышав «принято», LiveKit больше не пришёл бы, и упади первая доставка — событие пропало бы.
