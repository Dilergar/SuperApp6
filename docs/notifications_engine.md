...
- Ретеншн — шаги `notifications.rows` / `notifications.events` раннера сроков core/lifecycle (срок — политики `Notification` / `NotificationEvent` реестра, 90 дней; keyset-курсор, заморозка удерживает): не трогает `savedAt` и живой snooze; событие удаляется, когда не осталось строк адресатов. Крон модуля только отключает устройства без визита 60 дней. Дев-ручка `POST /dev/retention` зовёт раннер сразу. Строки удалённой организации уходят пачкой её каскада (по `workspaceId`). Стирание актора: снимок текста его событий (`snapshot`) снимается (`redactActorSnapshots`, джоб анонимизации).
