...
| `NotificationDelivery` (`notification_deliveries`) | **Леджер идемпотентности фанаута** + журнал: `recipient` (`user:<id>` \| `chat:<id>`), channel, status `queued/sent/delivered/failed/skipped`, `skipReason` (`pref_off/policy/muted/quiet/no_device/driver_not_configured/no_access/actor/throttled/seen/burst/no_phone/budget/expired`) | **месячные RANGE-партиции по `created_at`**, PK `(id, created_at)`, unique `(eventId, recipient, channel, created_at)`, BRIN по времени; `created_at` = момент СОБЫТИЯ (детерминирован — уникум работает и на ретрае в другом месяце); ретеншн 30 дней сбросом партиции (`core/lifecycle`) |
...
- Типы `security.*` (`notifications/security.ts`: новое устройство/страна, блокировка входа, заморозка/разморозка, «Это не я», забытое устройство, «выгрузка журнала готова», «массовая выгрузка данных в организации») рождает САМ журнал безопасности: личные — полем `notify` реестра событий в транзакции события (продюсер их не дублирует), организационные («выгрузка готова», «массовая выгрузка») — движок журнала своим вызовом `send`; SMS по ним — только человеку без живого push-устройства, без ссылок, не чаще раза в час (джоб `audit.sms_alert`) — [audit_engine.md](audit_engine.md).
...
- **Объём в payload — ЧИСЛОМ БАЙТ**, с суффиксом `Bytes` (`usedBytes: 4402341478`): рендер соберёт «4,1 ГБ» единицами и форматтером языка АДРЕСАТА под именем без суффикса; заданное `<имя>` строкой не перебивается.
...
- **SMS-бюджет организации — РЕЗЕРВ, а не списание вперёд.** Доставка берёт единицу тарифной квоты `notifications.smsPerDay` (`core/entitlements`) в транзакции ДО обращения к шлюзу (пачка параллельных доставок иначе проедет потолок) и ВОЗВРАЩАЕТ её `release` на каждом пути, где SMS не ушла: текст не собрался, шлюз бросил исключение (джоб ретраит — резерв возьмётся заново), шлюз ответил отказом. Отказ 402 от движка = доставка `skipped: budget`. Личный суточный потолок (`NOTIFICATION_LIMITS.smsPerUserDaily`, Redis) — анти-абьюз, не тариф: он пишется по факту успешной отправки.
- Ретеншн — шаги `notifications.rows` / `notifications.events` раннера сроков core/lifecycle (срок — политики `Notification` / `NotificationEvent` реестра, 90 дней; keyset-курсор, заморозка удерживает): не трогает `savedAt` и живой snooze; событие удаляется, когда не осталось строк адресатов. Крон модуля только отключает устройства без визита 60 дней. Дев-ручка `POST /dev/retention` зовёт раннер сразу. Строки удалённой организации уходят пачкой её каскада (по `workspaceId`). Стирание человека (шаг `notifications.subject` оркестратора, [lifecycle_erasure.md](lifecycle_erasure.md)): снимок текста его событий как актора снимается, а в событиях, где он — «о ком» (имя в payload парой с id: `targetUserId`/`otherUserId`/`fromUserId`/`byUserId`/`ownerUserId`), имя переписывается маркером томбстоуна и снимок тоже снимается (GIN-индекс `notification_person_ids`). **Имя человека в payload — только парой с его id**: `send()` проверяет (`personRefProblems`: в деве бросает, в проде — журнал); рендер показывает маркер томбстоуна меткой на языке зрителя.
...
**Видимость полей в уведомлениях.** Payload несёт только коды и id — ни значений защищённых полей, ни имён полей класса `personal` и выше (страж `check:visibility`). Если уведомление показывает поле записи (сумма, телефон), продюсер берёт его через `visibility.viewerFor(recipientId, …)` + `shape()` глазами ПОЛУЧАТЕЛЯ, а не автора — [visibility_engine.md](visibility_engine.md). Типы самого движка видимости: `visibility.reveal.notice` (субъекту: его данные раскрыли — по тумблеру организации), `visibility.policy.published`, `visibility.reveal.paused` (владельцу и админам).
