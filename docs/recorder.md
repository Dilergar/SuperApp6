...
- **Названия по умолчанию НЕТ в БД**: `title = null` означает «имя собирается при чтении» (`RecorderService.displayTitle`, render-at-read) — записанное автоимя навсегда осталось бы в языке, который был у человека в момент создания. Непустой `title` — только то, что человек ввёл руками. `titleAt` называет МОМЕНТ автоимени: у записи звонка это начало разговора (`createdAt` у неё — когда запись доехала из егресса), у остальных пусто → `createdAt`.
...
Неймспейс `recorder` (страница одностраничная, но `layout.tsx` с `<ServiceMessages ns="recorder">`
у неё есть — правило неймспейсов действует и здесь). Автоимя записи собирается ПРИ ЧТЕНИИ
в языке запроса (`recorder.defaultTitle` / `recorder.callTitle`, дата и время — общими
форматтерами), поэтому смена языка перерисовывает всю ленту записей. Единственное
исключение — снимок имени внутри УВЕДОМЛЕНИЯ (`titleFor`): у джоба языка запроса нет,
берётся язык адресата, и там имя застывает — лента уведомлений живёт неделями, а карточка
записи перерисуется при следующем чтении. Отказы — `errors.recorder.*`. Код языка, который
вернул STT, показывается САМОНАЗВАНИЕМ (`LOCALE_NAMES`), если он наш. `recorder.transcript.title`
по-казахски пишется латиницей (`Transcript`) — точечное решение продукта, см. глоссарий.
