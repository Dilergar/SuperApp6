...
### P0 (чинить первым — ломается само) — ✅ ВЫПОЛНЕНО 2026-07-01
> Все пункты сделаны и проверены: `verify-processes.cjs` ПРОЙДЕН (+ новая секция «P0 (ревью)»), регрессии tasks-access/order/order-fulfilment/messenger-task зелёные, web tsc чист. Не закоммичено (по просьбе пользователя). Два продуктовых решения: **A2 = «Конец» гасит только свою ветку** (branch-local; инстанс завершается по drain); **P3 = полный редизайн лока**.
- ✅ **A7+A3** — advance-фаза хуков под инстанс-локом (`withInstanceLock`) + partial-unique на join-шаги (миграция `20260701175000_process_join_unique`).
- ✅ **A1+A8** — join добивается eager (`tryForceFireStuckJoin`, под локом) + крон-подстраховка (`sweepStuckJoins`) через reachability (`canReach`); вечного зависания нет (продолжение, а не error).
- ✅ **A2** — `end` = `'consumed'` (гасит только свой токен), соседей/задачи не трогает; `completeInstanceIfDrained` завершает инстанс при 0 активных шагов.
- ✅ **A12** — `safeFetch`: числовой разбор IP (dec/hex/oct/short/IPv6-mapped) + DNS-резолв всех адресов + ручной follow с реперепроверкой хопов + снятие Authorization на кросс-хосте; применён к http-ноде, 1С, KZ-нодам (через `fetchJson`) и агентскому `http_get`. *Отложено (не P0): domain-allowlist агента (нужен ws-конфиг).*
- ✅ **A4** — маркер `source='process'` в payload `task.created` (закрывает гонку) + backstop-сверка «задача-шаг процесса» + бюджет `maxRunningInstancesPerWorkspace=1000` в `startInstanceProgrammatic`.
- ✅ **P8/A5** — монотонный `ProcessInstance.stepsSpawned` (миграция `20260701174928_process_review_p0_columns`) вместо `COUNT(*)`.
- ✅ **P3** — kick переписан: I/O-ноды (флаг `descriptor.io`) исполняются ВНЕ короткого инстанс-лока под арендой шага (`leasedUntil`, 200с); bookkeeping — под локом (TTL 30с).
- ✅ **NodeRunDeps-резолвер (ModuleRef)** — `deps.getService(token)` (strict:false); энейблер «всё как ноды» готов (новых нод пока не добавлял — это P1).

### P1 (нагрузка + ценные ноды/триггеры)
- ✅ **Перф (Фаза 1) — ВЫПОЛНЕНО 2026-07-02** (verify-processes ПРОЙДЕН + секция P7; регрессии зелёные; web tsc чист): **P1** buildContext (кэш имён на инстанс + done-шаги грузятся только при ссылке `{{steps.x}}`); **P2** recovery-крон (курсор-дренаж stale+waiting до опустошения, а не take(200)); **P4** eventType в колонку+индекс, роутер фильтрует индексом (fast-path in-memory set ОТКЛОНЁН — TTL-staleness ломал немедленный запуск после публикации; нужен shared-registry, отложено); **P5** getReport — агрегаты в SQL (raw `$queryRaw`, не 10k строк в JS); **P6** partial-индекс claimable-шагов `WHERE status='active' AND task_id IS NULL`; **P7** снимок `label` на шаге (журнал не парсит документы), getInstance без output-блобов, тонкий `GET /instances/:id/status` для 4с-поллинга + фронт-сплит (deталь 1×, статус — поллинг). 3 миграции. Не закоммичено.
- ✅ **Ноды-действия (Фаза 3) — ВЫПОЛНЕНО 2026-07-02** (verify-processes ПРОЙДЕН + секция «Фаза 3»; регрессии tasks/order/staff/messenger зелёные; web tsc чист). 5 безопасных нод через `ctx.deps.getService(token)`, ВСЕ от имени инициатора (`ctx.startedById`) — сервис перепроверяет права ИМЕННО инициатора, эскалации нет (проверено: не-Постановщик/trainee → ветка «Ошибка»): **action.richcard** (`RichCardsService.execute` — ~13 действий: order.confirm/reject/cancel/refund, task.accept/return/take, event.rsvp_*, listing.buy, crowdfunding.*; capability rechecked); **service.message** (openDm+sendMessage; +astool — инструмент агента); **staff.assign** (`assertStaffManage`); **workspaces.role** (`assertCanManage`); **process.start** (под-процесс same-ws + depth-guard `maxSubprocessDepth=5`). Добавлены строковые токены 'RichCardsService'/'WorkspacesService'/'StaffService'; `ProcessesService.startSubprocess`; поля-пикеры `position`/`branch` в NDV. Не закоммичено. **Отложено (нужна доработка сервиса перед экспонированием):** `wallet.payEmployee` — owner-check ТОЛЬКО в контроллере (нода = эскалация; поднять в сервис, деньги — отдельно); `resources.book`/`access.check/list_objects`/`workspaces.inviteMember` — по-сервисная проверка/меньший приоритет.
- ✅ **Триггеры (Фаза 4) — ВЫПОЛНЕНО 2026-07-02** (verify-processes ПРОЙДЕН + секция «Фаза 4»; регрессии order/order-fulfilment/tasks/messenger зелёные; web tsc чист). **Реестр резолверов** `eventPrefix→резолвер` в ProcessTriggerRouter (новое семейство = +1 запись). Новые события: **shop.order.placed/funded/confirmed** (эмитятся ✓; резолвер order→shop→workspace, только company-shop ownerType='workspace') + **workspace.member.removed** (эмитится ✓ с workspaceId — уже reuse workspace-резолвера; «увольнение → offboarding-процесс», проверено e2e). Не закоммичено. **Уточнение к ревью (сверено с кодом):** `message.created` НЕ на шине (мессенджер эмитит только presence) — не «уже на шине», как в ревью; `mention.created`/`calendar.event.created` тоже НЕ эмитятся. **Отложено:** messenger.message.created (нужен эмит + анти-луп: `service.message`-нода шлёт сообщения → петля; + high-freq), mention.created (нужен эмит), calendar.event.created (нет эмита + события личные). `workspace.member.fired` из ревью = фактически `workspace.member.removed` (публичный путь увольнения).
- ✅ **Надёжность (Фаза 2) — ВЫПОЛНЕНО 2026-07-02** (verify-processes ПРОЙДЕН + секция «Фаза 2»; регрессии зелёные; web tsc чист): **n8n#4 onError** (универсальное поле «При ошибке» stop/continue/errorOutput — умолчание stop = как раньше; throw ноды → onError-маршрутизация, а не безусловный failInstance); **n8n#3 Retry On Fail** (`retryMaxTries`/`retryWaitMs` на io-нодах, повтор ВНЕ лока, `output._retries`); **sfflow#3 fault-port** (throw больше не всегда валит инстанс; `notify` — best-effort, сбой не роняет процесс); **sfflow#1 entry-conditions** (поле события `condField/condOp/condValue` на триггере «Событие» — фильтр payload до старта, reuse `evalCondition`). Универсальные поля авто-рендерятся в NDV (append в `registry.listTypes`), извлекаются компилятором из raw config. Не закоммичено.
- ✅ **Поток данных (Фаза 5, n8n#1/#2/#5) — ВЫПОЛНЕНО 2026-07-02** (verify-processes ПРОЙДЕН + секция «Фаза 5»; регрессии tasks/order/messenger зелёные; web tsc чист). Решения пользователя: **items[]** как модель данных + **безопасный язык выражений** (не JS) + **явная нода «Перебрать список»** для размножения (человеческие шаги у нас есть — у n8n нет). Сделано: (1) `NodeRunResult.setVariables` — движок мержит поля в анкету инстанса (advance-tx); `ctx.resolveValue`/`item`-контекст (`{{item.поле}}` из `variables._item`). (2) **«Перебрать список» (loop.each)** — последовательный re-entrant цикл (модель n8n Loop Over Items): источник→массив, индекс в `variables._loopIdx_<nodeId>`, текущий элемент в `_item`, выходы «Каждый»/«Готово», ветка «Каждый» возвращается связью в ноду; `maxStepsPerInstance` поднят 500→2000. (3) **Безопасный вычислитель** `process-expression.ts` (AST, без eval; арифметика/сравнения/логика/тернарник/доступ к полям+индексам/функции len,upper,lower,trim,round,number,string,contains,default) — бенчмарк CEL/Salesforce-формулы; `renderTemplate` использует его (плоский путь — быстрый путь, ноль риска для старых шаблонов). (4) **«Задать данные» (data.set)** — вычисляет поля → setVariables. Проверено e2e: `round(form.sum*0.12,2)=12`; вебхук-массив → цикл `{{item.n}}*2` на каждый; пустой список → «Готово». **Отложено:** параллель-на-элемент (сейчас последовательно — норм для интеграций; «N задач разом» — позже); нода «Фильтр»/условие-на-item (нужен per-element eval). Не закоммичено.

### P2 (удобство, потом)
✅ **Полировка (Фаза 6) — ВЫПОЛНЕНО 2026-07-02** (verify-processes ПРОЙДЕН + секция «Фаза 6»; регрессии зелёные): **A6** детект циклов агентов-инструментов в компиляторе (DFS по ai_tool-графу); **sfflow#4** сбой процесса уведомляет и создателя определения (ответственного), не только инициатора; **P10** LRU planCache (move-to-front); **P12** AI-память на Redis LIST (rpush/lrange/ltrim — нет потери реплик при гонке); **A11** расписание не крутит nextRunAt у неопубликованных. Не закоммичено.
**Осталось (фичи, отдельно):** dry-run/pinned-data; subflow (частично закрыт нодой process.start Ф3); resume-by-webhook; error-workflow (отдельный процесс на сбой); anchored-расписание (запуск в конкретный час); schemaVersion+recompile в движке (A10 — нужна инъекция реестра); async webhook-ACK (меняет контракт ответа); ретенция версий; read-tools агента; **+ отложенное из Фаз 3–5:** wallet.payEmployee (поднять owner-check в сервис), триггеры message/mention/calendar (нужен эмит + анти-луп), нода «Фильтр»/параллель-на-элемент.
