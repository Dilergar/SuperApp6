...
export interface InboxScope {
  ...
  /**
   * Организации в архиве, где человек состоит, — их задания источник НЕ показывает
   * (сервисы выключенной организации закрыты; возврат из архива возвращает задания).
   * Заполняет движок стопки, не клиент: на проводе этого поля нет.
   */
