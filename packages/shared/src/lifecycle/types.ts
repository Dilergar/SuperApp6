...
export interface LifecyclePolicy {
  ...
  /**
   * Чьи строки уходят в выгрузку стороны, когда ключ владельца её не называет: строка кадров
   * организации — в архив человека по колонке сотрудника, заказ магазина организации — в
   * архив организации через магазин. Иначе сторона отбирается ключом владельца (человек,
   * полиморфный владелец, организация с человеком, родитель по `via`) либо провайдером
   * модуля; ни то ни другое — смоук бута падает.
   */
  exportScope?: { user?: LifecycleExportScope; workspace?: LifecycleExportScope };
  /**
   * Поля строки под правилами видимости (core/visibility): сборщик выгрузки проецирует их
   * `shape()` глазами заказчика — маска так и уходит маской, скрытое не уходит. Архив
   * организации не может стать обходом маски ИИН или оклада (массовое раскрытие без SMS).
   */
  exportGuard?: readonly LifecycleExportGuard[];
  /**
...
/**
 * Область выгрузки стороны: строка принадлежит субъекту, если ЛЮБАЯ колонка `columns` равна
 * его id либо колонка `via.columns` указывает на строку политики-родителя этого же субъекта
 * (область родителя — своя, по его ключу владельца). `filter` — условие строки (как у раннера),
 * `personalOnly` — только строки вне организации.
 */
export interface LifecycleExportScope {
  columns?: readonly string[];
  via?: { policy: string; columns: readonly string[] };
  filter?: LifecycleRowFilter;
  personalOnly?: boolean;
}

/**
 * Поля строки, которые решает реестр видимости: `fields` — колонка → поле типа, `subject` —
 * колонка человека записи («сам» и руководитель), `branch` — колонка объекта, `stage` — этап.
 */
export interface LifecycleExportGuard {
  type: string;
