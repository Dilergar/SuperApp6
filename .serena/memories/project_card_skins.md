...
- Статус: построено + хардненинг; НЕ закоммичено (в рабочем дереве), в браузере хардненинг визуально НЕ отсмотрен. Дальше: F2/F6 (по запросу), платёжные рельсы, трейд/подарки, регистрация dept/position/branch в EntitySelector, @username-чип, доп. скины. См. CLAUDE.md + авто-память.
